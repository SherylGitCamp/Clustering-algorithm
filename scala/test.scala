...
//        val columnNames = dataRemain.take(1).flatMap(a => a.keys)
     //@timestamp:2018-01-08 AND @timestamp:{2018-01-08 TO 2018-01-19} AND
    val data2018DF = sqlContext.esDF("data2018", "?q=mediaType:video AND reportType:sender")

     println( data2018DF.count())
...
//    val clusters = KMeans.train(dataRemainDF,numClusters,numIterations)//train(dataRemainDF, numClusters, numIterations)
    ...
    results.drop("features").drop("scaledFeatures").write.json("/home/v/Desktop/kmeansCSV/") //format("csv").save("/home/v/Desktop/kmeansCSV")

    //split the data 70% for training and others for validation
    val splitSeed = 5043
    val Array(trainingData, testData) = results.randomSplit(Array(0.7, 0.3), splitSeed)
    ...
    predictions.select("scaledFeatures", "label", "prediction").show(500)
...
//    results.where("prediction == 9").toDF.show(1500)
//    println(results.where("prediction == 8").toDF().count())
//    results.where("prediction == 8").toDF.show(1500)
//    println(results.where("prediction == 7").toDF().count())
//    results.where("prediction == 7").toDF.show(1500)
//    println(results.where("prediction == 6").toDF().count())
//    results.where("prediction == 6").toDF.show(1500)
//    println(results.where("prediction == 5").toDF().count())
//    results.where("prediction == 5").toDF.show(1500)
//    println(results.where("prediction == 4").toDF().count())
//    results.where("prediction == 4").toDF.show(1500)
//    println(results.where("prediction == 3").toDF().count())
//    results.where("prediction == 3").toDF.show(1500)
//    println(results.where("prediction == 2").toDF().count())
//    results.where("prediction == 2").toDF.show(1500)
//    println(results.where("prediction == 1").toDF().count())
//    results.where("prediction == 1").toDF.show(1500)
//    println(results.where("prediction == 0").toDF().count())
//    results.where("prediction == 0").toDF.show(1500)
