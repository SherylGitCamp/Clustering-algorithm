...
//"googRtt", "googAdaptationChanges", "googAvgEncodeMs", "googEncodeUsagePercent", "googFirsReceived", "PL", "Nacks", "Plis"

var RTT_array = new Array()
var PL_array = new Array() 
var NACK_array = new Array()
var Plis_array = new Array()
var googAdaptationChanges_array = new Array()
var googAvgEncodeMs_array = new Array() 
var googEncodeUsagePercent_array = new Array()
var googFirsReceived_array = new Array()

var old_PL = 0
var old_NACK = 0
var old_Plis = 0
var old_googAdaptationChanges = 0
var old_googFirsReceived = 0

var current_Rtt = 0
var current_googEncodeUsagePercent = 0
var current_googAvgEncodeMs = 0
var current_read_PL = 0
var current_read_NACK = 0
var current_read_Plis = 0
var current_read_googAdaptationChanges = 0
var current_read_googFirsReceived = 0

var true_PL = 0
var true_NACK = 0
var true_Plis = 0
var true_googAdaptationChanges = 0
var true_googFirsReceived = 0

var videoSenderIndex 
var videoExist = true
var socket
var Message_map = { 	 "4":"Good!", 
			 "3":"Recommand: Drop the frame rate in the video OR Reduce the resolution to get a much clear and correct video in return.", 
			 ...
			 "1":"The current video quality is extremely horrible, we strongly recommend you drop the video completely to keep audio fluency.", 
...
if(window.WebSocket){
	...
	socket.onopen = function(event){
		console.log("your browser support WebSocket,continue please\r\n")
	};
...
class WebRTC extends Component {
	...
	console.log(current_Rtt)
	  if ( !isNaN(current_Rtt) && current_Rtt != 0) {


//"googRtt", "AdaptationChanges", "googAvgEncodeMs", "googEncodeUsagePercent", "googFirsReceived", "PL", "Nacks", "Plis"
		console.log('ssrc_videoSender.RTT:', current_Rtt)	  
		RTT_array.push(current_Rtt)

		current_read_googAdaptationChanges = parseInt(Object.values(e)[videoSenderIndex].googAdaptationChanges)
		if (current_read_googAdaptationChanges >= old_googAdaptationChanges){
		true_googAdaptationChanges = current_read_googAdaptationChanges - old_googAdaptationChanges
		}
		else{
		true_googAdaptationChanges = current_read_googAdaptationChanges
		console.log("you change setting here...")
		}
		console.log('ssrc_videoSender.googAdaptationChanges:', true_googAdaptationChanges)
		googAdaptationChanges_array.push(true_googAdaptationChanges)
		old_googAdaptationChanges = current_read_googAdaptationChanges

		console.log('ssrc_videoSender.googAvgEncodeMs:', current_googAvgEncodeMs)	  
		googAvgEncodeMs_array.push(current_googAvgEncodeMs)

		console.log('ssrc_videoSender.googEncodeUsagePercent:', current_googEncodeUsagePercent)	  
		googEncodeUsagePercent_array.push(current_googEncodeUsagePercent)


		current_read_googFirsReceived = parseInt(Object.values(e)[videoSenderIndex].googFirsReceived)
		if (current_read_googFirsReceived >= old_googFirsReceived){
		true_googFirsReceived = current_read_googFirsReceived - old_googFirsReceived
		}
		else{
		true_googFirsReceived = current_read_googFirsReceived
		console.log("you change setting here...")
		}
		console.log('ssrc_videoSender.googFirsReceived:', true_googFirsReceived)
		googFirsReceived_array.push(true_googFirsReceived)
		old_googFirsReceived = current_read_googFirsReceived
    ...
    timer_sending_parameters() {
      ...
      if(!videoExist) {
    	  ...
    	  RTT_array = []
	  googAdaptationChanges_array =  []
  	  googAvgEncodeMs_array =  []
	  googEncodeUsagePercent_array =  []
	  googFirsReceived_array =  []
    	  PL_array = []
    	  NACK_array = []
    	  Plis_array = []
    	  return         
    	...
    	if(RTT_array.length != 0){
            	send([RTT_array,googAdaptationChanges_array,googAvgEncodeMs_array,googEncodeUsagePercent_array,googFirsReceived_array,PL_array,NACK_array,Plis_array])
    	}
    	RTT_array = []
	googAdaptationChanges_array =  []
	googAvgEncodeMs_array =  []
    ...
    handleClickStop() {
      if(this._start) {
        ...
        this.source1 = null
        this.source2 = null
        this.client1 && this.client1.logout()
        this.client2 && this.client2.logout()
        this.client1 = null
        this.client2 = null
  ...
  setTimeout(this.handle_index,7000)  
	var qos = document.getElementById('qosContent');
    ...
    render() {
     ...
     // console.log('the value is:' + !!(this.state && this.state.open) || false)
	  ...
	  <form>
		...
		Note: video quality ranking: <mark>the best 4</mark> --> 3 --> 2 --> 1 --> <mark>0 the worst</mark>
	                               
            <h2>Remedy Descriptions:</h2>
